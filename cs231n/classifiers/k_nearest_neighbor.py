...
class KNearestNeighbor(object):
  ...
  def compute_distances_no_loops(self, X):
    ...
    dists = -2 * np.dot(X, self.X_train.T) + np.reshape(np.sum(X*X, axis = 1), (num_test,1)) + np.reshape(np.sum(self.X_train*self.X_train, axis = 1), (1, num_train))
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      #这里将closest_y定义为list是有用处的，一开始我就觉得奇怪，其他变量全是array，为什么突然冒出来一个list
      ...
      for j in xrange(len(labels)):
         counts.append(closest_y.count(labels[j]))
      y_pred[i] = np.argsort(-np.array(counts))[0]
