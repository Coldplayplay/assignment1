...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    scores_p_labels = scores_p[(range(0, N), y)]
    loss = np.sum(-np.log(scores_p_labels))/N + reg * (np.sum(W1*W1) + np.sum(W2*W2))    
    #############################################################################
    ...
    dW2 = h1.T.dot(dscores) + reg * 2 * W2
    db2 = np.ones(N).dot(dscores)
    
    dW1 = X.T.dot((h1>0) * dh1) + reg * 2 * W1
    db1 = np.ones(N).dot((h1>0) * dh1)
